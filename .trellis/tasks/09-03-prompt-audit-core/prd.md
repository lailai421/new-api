...
- [x] 配置默认关闭，启用时至少有一个可用节点、一个风险分类及有效分组范围。
- [x] 未配置稳定 `CRYPTO_SECRET`/`SESSION_SECRET` 时无法启用。
- [x] 节点 Token 和提示词正文使用不同用途密钥加密，密文可完整解密。
- [x] 配置存在但解析、解密或激活失败时进入 degraded，不回退为关闭。
- [x] Safe、Controversial、Unsafe、未知分类和九类 scanner 判定符合父设计。
- [x] 429、5xx、网络失败和超时按节点顺序切换；4xx 与非法响应不被错误重试。
- [x] 超长 Unicode 输入全部分片并按最高风险聚合，任一 Block 可提前终止。
- [x] 核心日志不包含 Token、提示词正文或 Guard 响应正文。
- [x] 单元测试使用 testify，覆盖真实行为边界，不添加随机/计时型伪测试。
