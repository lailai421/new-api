...
- Trellis 任务：`.trellis/tasks/09-04-prompt-audit-codex-cli-only`，父任务 `09-03-prompt-audit`，实现与检查已完成，状态 `in_progress`；未 commit、未 archive。
- 实现落点：`service/promptaudit/client.go` 的 `IsCodexCLIRequest` / `CheckAuditClientAccess`；错误码与消息在 `types.go`。

- 用户已确认采用兼容性识别：入站 `Originator` 是硬判定依据，User-Agent/Session/X-Codex 等辅助头不得单独授权；明确接受请求头可伪造的风险，不做签名/设备证明。
- Originator 完整值 allowlist：规范化后仅 `codex_cli_rs`、`codex-cli`、`codex-tui`、`codex_exec`；Trim + ASCII 大小写不敏感，不做子串、正则或通配符。交互 TUI 发送 `codex-tui`，`codex exec` 发送 `codex_exec`。
...
- 规划文档已齐且用户已批准实施；实现与 trellis-check 已完成。详细落点见 `prompt_audit_codex_cli_only.md`。